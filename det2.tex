...
\noindent Thus the parallelogram shown above is the box formed by 
$\sequence{\binom{x_1}{y_1},\binom{x_2}{y_2}}$.
A three-space box is shown in \nearbyexample{ex:VolParPiped}.

We can compute the area of the above box by drawing an enclosing rectangle
and subtracting away areas of subregion not in the box.
\begin{center}
...
As before, this is a determinant condition.

Lastly, we expect that the box formed by unit vectors has unit size
\begin{center}
...
That's the last condition to check because 
condition~(2) is 
redundant, as remarked following the definition of determinant. 
So we have that the properties of the determinant function are reasonable
for function that gives the size of boxes.
...
% We will prove the second sentence, the matrix version, because 
% it is more convienent for the argument 
% and also because it is the way that we shall later use this result.
% (Alternate proofs are in 
...
\( T(SM)=I \) and so \( T \) is invertible, with inverse $SM$.
By contrapositive, 
