...
% I put in a fancy title, just for fun.  But it can be disabled
% because it really works best with postscript fonts 
...
\iffancytitle
  ...
  \begin{center}
    \begin{picture}(0,0)(0,0)
      ...
      \put(0,-350){\makebox[0pt]{\large\scshape\begin{tabular}{c} Jim Hef{}feron \\ 2003-May-05\end{tabular}}}
    \end{picture}
  \end{center}
...
%followed by `\verb!dvips -Pwww -ojhanswer.ps jhanswer!', 
...
The steps below work for me.
I make a new directory \textit{book} and change into it.
I \verb!unzip linear_algebra!, and then also unzip each sub-archive.  
...
\begin{center}
  \begin{tabular}{ll}
    ...
    \verb!pdflatex book!  &Produce the book's reference files.       \\
    \verb!makeindex -s book.isty -p odd book.idx!  &Alphabatize index.  \\ 
    \verb!pdflatex book!  &Produce the book's .dvi file.            %   \\
    % \verb!dvips -Pwww book!  &Produce the book in PostScript form. 
  \end{tabular}
\end{center}

To get the version on the web I substitute this.
\begin{quotation}
    \verb!pdflatex "\def\dvidrv{dvipdfm}\def\pbsifont{yes}\def\hrefout{yes}\input book"!
\end{quotation}
%(I had to get the \textit{dvipdfm} DVI driver and the 
%\textit{BrushScriptItalic} font from CTAN; see the final section of this 
%document).
I then run the same command with \textit{jhanswer} instead of 
\textit{book} to get the answers.
(I don't like the way the hyperlinked colors print, so for the printable
version I drop the links.)

As mentioned above, 
all these steps are in the unix shell script \verb!make_book_for_web.sh!.
