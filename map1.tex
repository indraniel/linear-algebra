...
the intuition that these spaces differ only in how their elements appear.
...
\end{equation*}
This is an example of the correspondence respecting scalar multiplication.
\begin{equation*}
...
However we shall do second one,
...
that rotates all vectors through an angle of $\theta$ is also an automorphism.
%</ex:RigidPlaneMapsAutos1>
\begin{center} \small
...
We shall prove that the relationship `is isomorphic~to' is an equivalence
...
Suppose $\vec{w}_1,\vec{w}_2\in W$.
Because it is an isomorphism, $f$ is onto and
so there are $\vec{v}_1, \vec{v}_2\in V$ such that
...
With that, by
\nearbylemma{le:PresStructIffPresCombos}'s second item this map preserves
structure.
%</pf:IsoInvAlsoIso1>
\end{proof}
...
% For these we will use item~(2) of 
% \nearbylemma{le:PresStructIffPresCombos} to verify that the map preserves
% structure.
...
As an equivalence, isomorphism partitions the universe of vector spaces 
...
\begin{center} \small
  ...
  \raisebox{.4in}{\begin{tabular}{l}
                  ...
                  \end{tabular}}
\end{center}
The next result gives a simple characterization of the classes.
That is, we can describe what the classes are, and decide
to which class a space belongs.  
...
In this double implication statement both of the implications 
take some space to prove, so we will do the two halves separately.
