...
that the map must preserve the algebraic structure of the space.
We will focus on this condition by studying maps that 
are required only to preserve structure;
that is, maps that are not required to be correspondences. 

Experience shows that this kind of map is 
tremendously useful in the study of vector spaces.
For one thing, as we shall see in the second subsection below, 
while isomorphisms describe how spaces are the same,
...
\end{equation*}
is a homomorphism.
%The check is familiar from the prior section.
...
might be other than spaces of column vectors.
...
\begin{enumerate}
  \item The map \( \map{g}{\Re^3}{\Re} \) given by
    \begin{equation*}
      ...
      3x+2y-4.5z
    \end{equation*}
    is linear (i.e., is a homomorphism).
    ...
    is not;~for instance,
    \begin{equation*}
      \hat{g}(\colvec{0 \\ 0 \\ 0}+\colvec{1 \\ 0 \\ 0})=4
      \quad\text{while}\quad
      \hat{g}(\colvec{0 \\ 0 \\ 0})
      +\hat{g}(\colvec{1 \\ 0 \\ 0})=5
    \end{equation*}
    (to show that a map is not linear we need only produce one example of
...
\end{enumerate}
What distinguishes the homomorphisms is that the
coordinate functions are linear combinations of the arguments.
See also \nearbyexercise{exer:GrpahNotALine}.
\end{example}

Obviously, any isomorphism is a
homomorphism\Dash an isomorphism is a 
homomorphism that is also a correspondence.
So, one way to think of the `homomorphism' idea 
is that it is a generalization of `isomorphism',
...
and not to do with it being a correspondence.
As examples, these two results from the prior section
do not use one-to-one-ness or onto-ness in their proof, 
and therefore apply to any homomorphism.
...
\( \map{f}{V}{W} \) to be a homomorphism.
\begin{enumerate}
  \item 
...
\end{enumerate}
\end{lemma}
Part~(1) is often used to check that 
a function is linear.

\begin{example}
The map \( \map{f}{\Re^2}{\Re^4} \) given by
\begin{equation*}
  \colvec{x \\ y}
    \mapsunder{f}
  \colvec{x/2 \\ 0 \\ x+y \\ 3y}
...
\nearbyexample{ex:RThreeHomoRTwoFirst} shows that there 
is no such correspondence, and another example is 
the zero map between any two nontrivial spaces. 
...
\( \vec{w}_1,\dots,\vec{w}_n \) are (perhaps not distinct) elements of
a vector space \( W \) then
there exists a homomorphism from \( V \) to \( W \) sending
\( \vec{\beta}_1 \) to \( \vec{w}_1 \), \ldots, and
\( \vec{\beta}_n \) to \( \vec{w}_n \), and that homomorphism is unique.
\end{theorem}

\begin{proof}
We will define the map by 
associating $\vec{\beta}_1$ with $\vec{w}_1$, etc.,
and then extending linearly to all of the domain.
That is, where 
\( \vec{v}=c_1\vec{\beta}_1+\dots+c_n\vec{\beta}_n \), the map
\( \map{h}{V}{W} \) is given by 
...
\begin{equation*}
  h(\colvec{1 \\ 0})=\colvec{-1 \\ 1}
  \quad\text{and}\quad
  h(\colvec{0 \\ 1})=\colvec{-4 \\ 4}
\end{equation*}
then the action of $h$ on any other member of the domain is also
specified.
For instance,
the value of $h$ on this argument
\begin{equation*}
  h(\colvec{3 \\ -2})=h(3\cdot \colvec{1 \\ 0}-2\cdot \colvec{0 \\ 1})
                      =3\cdot h(\colvec{1 \\ 0})-2\cdot h(\colvec{0 \\ 1})
                      =\colvec{5 \\ -5}
\end{equation*}
is a direct consequence of the value of $h$ on the basis vectors.

Later in this chapter we shall develop a scheme, using matrices,
that is convienent for computations like this one.
\end{example}
...
the codomain equals the domain, but it is widely used in other texts as a
general synonym for `homomorphism'.
\end{remark}
...
Note that this transformation is one-to-one and onto, and so in fact it is 
an automorphism.
\end{example}
...
We started this section by isolating the structure preservation property
of isomorphisms.
That is, we defined homomorphisms as a generalization of isomorphisms.
Some of the properties that we studied for isomorphisms carried over unchanged,
while others were adapted to this more general setting.

It would be a mistake, though, to view this new notion of 
homomorphism as derived from, or somehow secondary to,
that of isomorphism.
...
while the isomorphism concept is perhaps more natural, 
experience shows that the homomorphism concept 
is actually more fruitful and more central to further progress.
...
\begin{exercises}
  \recommended \item
    ...
    \begin{exparts*}
      ...
      \partsitem \( h(\colvec{x \\ y \\ z})=\colvec{0 \\ 0}  \)
      \partsitem \( h(\colvec{x \\ y \\ z})=\colvec{1 \\ 1}  \)
  ...
  \recommended \item
    ...
    \begin{exparts}
      \partsitem \( h(\begin{pmatrix} a &b  \\ c &d \end{pmatrix})=a+d  \)
      \partsitem \( h(\begin{pmatrix} a &b  \\ c &d \end{pmatrix})=ad-bc  \)
      \partsitem \( h(\begin{pmatrix} a &b \\ c &d \end{pmatrix})=2a+3b+c-d  \)
      \partsitem \( h(\begin{pmatrix} a &b  \\ c &d \end{pmatrix})=a^2+b^2  \)
    \end{exparts}
    \begin{answer} 
  ...
  \recommended \item 
    ...
    \begin{equation*}
      \colvec{x \\ y \\ z}\;\mapsto\;
      \colvec{x \\ y \\ z}\dotprod\colvec{3 \\ -1 \\ -1}=3x-y-z
    \end{equation*}
    is linear.
    Generalize.
    \begin{answer}
...
We first consider the effect of dropping the onto requirement, 
of not requiring as part of the definition that a homomorphism be
onto its codomain.
For instance, the injection map \( \map{\iota}{\Re^2}{\Re^3} \)
\begin{equation*}
  \colvec{x \\ y} \mapsto \colvec{x \\ y \\ 0}
\end{equation*}
is not an isomorphism because it is not onto.
Of course, being a function,
a homomorphism is onto some set, namely its range;
the map $\iota$ is onto the $xy$-plane subset of \( \Re^3\).
% The next result says that for any  range set is a vector space.
...
\end{lemma}
% add exercise that if h:V->W is a map then we can define +, * on W to make
% h a homo?
...
\( \rangespace{h}=\set{r+0x+sx^2+sx^3\suchthat r,s\in\Re} \)
and so the rank is two.
\end{example}
...
Above, the three sets of many elements on the left
are inverse images.
...
we lose the property that we've stated intuitively as: the 
domain is ``the same as'' the range.
That is, we lose that the domain
corresponds perfectly to the range
in a one-vector-by-one-vector way.
%Now, domain no longer corresponds to the codomain; 
%in the first example there is no
%single vector in $\Re^3$ that matches with the vector shown in
%$\Re^2$,
%and in the second example there is no vector in the plane that
%corresponds to $a$.
What we shall keep, as the examples below illustrate,
is that a homomorphism 
describes a way in which
the domain is ``like'', or ``analgous to'', the range.

\begin{example}    \label{ex:RThreeHomoRTwo} %\label{exPicProj}
We think of $\Re^3$ as being like $\Re^2$, except that vectors have an extra
component. 
...
is easiest if we draw $\Re^2$ as the $xy$-plane inside of $\Re^3$.
...
%vectors in \( \Re^3 \) act like their shadows in the plane in that
\begin{center}  \small
...
gives an uglier picture but one that is more faithful to the
``bean'' sketch.
\begin{center}  \small
  \includegraphics{ch3.4}
\end{center}
Again in this drawing, the vectors that map to $\vec{w}_1$ lie in the domain
in a vertical line
(only one such vector is shown, in gray).
Call any such member of this inverse image %$\pi^{-1}(\vec{w}_1)$ 
a ``$\vec{w}_1$~vector''.
...
then $\pi(\vec{v}_1+\vec{v}_2)=\pi(\vec{v}_1)+\pi(\vec{v}_2)
  =\vec{w}_1+\vec{w}_2$.
This says that the vector classes add, in the sense that 
any \( \vec{w}_1 \)~vector plus any
\( \vec{w}_2 \)~vector 
equals a \( \vec{w}_1+\vec{w}_2 \)~vector,
(A similar statement holds about the classes under scalar multiplication.)

Thus, although the two spaces $\Re^3$ and $\Re^2$ are not isomorphic,
...
A homomorphism can be used to express
...
Similarly, we have the ``$w_2$ vectors'' and the  ``$w_1+w_2$ vectors''.  
Then the addition preservation property says that
\begin{center}  \small
  \begin{tabular}{@{}ccccc@{}}
...
However, there is nothing bad about gaining insights from those spaces
that we are able to draw, 
especially when those insights extend to all vector spaces.
...
and let $S$ be a subspace of the rangespace $h$.
Consider $h^{-1}(S)=\set{\vec{v}\in V\suchthat h(\vec{v})\in S}$, 
the inverse image of the set $S$.
It is nonempty because it contains $\zero_V$, since
\( h(\zero_V)=\zero_W \), which is an element $S$, as $S$ is a subspace.
To show that $h^{-1}(S)$ is closed under linear combinations, let
\( \vec{v}_1 \) and \( \vec{v}_2 \) be elements,
so that $h(\vec{v}_1)$ and $h(\vec{v}_2)$ are elements of $S$,
and then $c_1\vec{v}_1+c_2\vec{v}_2$
is also in the inverse image because
$h(c_1\vec{v}_1+c_2\vec{v}_2)
  =c_1h(\vec{v}_1)+c_2h(\vec{v}_2)$
is a member of the subspace $S$. 
\end{proof}
...
In \nearbyexample{ex:RThreeHomoRTwo}, each of the vertical lines is squashed down 
...
\begin{example}
This nonsingular homomorphism \( \map{\iota}{\Re^2}{\Re^3} \)
\begin{equation*}
  \colvec{x \\ y}
    \mapsunder{\iota}
  \colvec{x \\ y \\ 0}
\end{equation*}
gives the obvious correspondence between \( \Re^2 \) and the \( xy \)-plane
inside of \( \Re^3 \).
\end{example}
...
\begin{tfae}
  \item \( h \) is nonsingular, that is, one-to-one
  \item \( h \) has a linear inverse
...
We've now seen that a linear map
...
\begin{exercises}
  ...
  \recommended \item 
    ...
    \end{answer}
  \recommended \item 
    Each of these transformations of \( \polyspace_3 \) is nonsingular.
  ...
  \item \label{exer:NonSingIffPreservLI} 
    Show that a linear map is nonsingular if and only if it preserves 
    linear independence.
    \begin{answer}
