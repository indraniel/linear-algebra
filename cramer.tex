...
\end{equation*}
Below we picture that vector equation.
It starts with a small parallelogram whose sides are the vectors 
$\binom{1}{3}$ and $\binom{2}{1}$.
That is shown nested inside another parallelogram, 
whose sides are $x_1\binom{1}{3}$ and $x_2\binom{2}{1}$ 
and whose far corner is, by the equation, the vector $\binom{6}{8}$.
%</CramersRuleExample0>
\begin{center}
  \includegraphics{ch4.1}
\end{center}
%<*CramersRuleExample1>
So we can restate the algebraic question of
finding the solution of a linear system
in geometric terms:~by 
what factors $x_1$ and~$x_2$ must we dilate the vectors of the starting 
parallelogram so that it will fill the other one?
%</CramersRuleExample1>

We can apply the geometric understanding of determinants
...
In this way we can by-eye solve systems that are small and simple. 
...
Such cases appear often enough that many people find Cramer's Rule handy.
 
But using it to solving large or complex systems is not practical,
either by eye or by a computer.
A
Gauss's Method-based approach is the best way to solve such systems.
