...
in the special case of systems with two equations and two unknowns.
...
(the word is Latin for ``carrier'' or ``traveler'').
...
\end{equation*}
so that for the `one over and two up' arrows shown above we write the vector 
this way.
\begin{equation*}
  \colvec[r]{1 \\ 2}
\end{equation*}
...
\end{center}
As shown, the vector associated with the parameter~\( t \)
\begin{equation*}
...
Note that for any nonzero $\vec{v}$, the vector $\vec{v}/\absval{\vec{v}}$
has length one.
...
This result is the source of the familiar saying, 
