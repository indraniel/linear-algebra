...
That is,
we want a \( B \) so that \( \rep{t}{B,B} \) is as simple as possible, where
we take `simple' to mean that it has many zeroes.
...
this first section is a quick review of complex numbers.
%</ReasonForShiftToComplexNumbers>
...
\textit{This subsection is a review, without proofs.
  See \cite{BirkhoffMaclane}, \cite{Ebbinghaus},
  or \cite{Derksen}.}
...
Similarly, the point of the polynomial division statement
is its final clause.
...
To determine the constant, take the theorem's $d(x)$ to be $x-\lambda$ and 
substitute \( \lambda\/ \) for $x$ to get
\( p(\lambda)=(\lambda-\lambda)\cdot q(\lambda)+r(x) \). 
%</pf:PolyDividedByLinearPolyIsConstant>
\end{proof}
...
then \( d(x) \) is a factor\index{factor}\index{polynomial!factor} 
of \( p(x) \).
...
with real number coefficients is irreducible over the 
...
respect to addition, multiplication, etc.\ (i.e., we also adjoin to the reals
all linear
combinations of $1$ and $i$ such as 
\( 3+i \), and \( -1.5i \), and \( -1-2i \), etc.)
then we get the 
...
We shall also carry over unchanged from the previous chapters
