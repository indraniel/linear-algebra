...
{Four.III Laplace's Expansion and Cramer's Rule}
...
\subject{Laplace's Expansion, Cramer's Rule}
...
Let $A$ be a square matrix, let $\vec{b}$ be an $n$-tall column vector,
and consider the linear system $A\vec{x}=\vec{b}$.
For any column number~$i$, let $B_i$ be the matrix obtained by
