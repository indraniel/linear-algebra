...
we express the same-ness of the spaces by 
...
on two inputs, and then show the two inputs must be equal.
...
We usually verify this by taking an element of the codomain $\Re^3$
and producing an element of the domain that maps to it.
So consider this member of $\Re^3$.
\begin{equation*}
  \colvec{u \\ v \\ w}
\end{equation*}
Observe that it is the image under $f$ of this member of the domain:
$u+vx+wx^2$.
Thus $f$ is onto.
\end{frame}
\begin{frame}
