...
\pause
\ex
This system is also already in echelon form.
\begin{equation*}
  \begin{linsys}{4}
   -2x  &+  &y  &-  &z   &+   &w  &= &3/2  \\
        &   &   &   &2z  &-   &w  &= &1/2 
  \end{linsys} 
\end{equation*}
Here the free variables are $y$ and~$w$, so we 
parametrize the solution set with those.
\pause
The second row gives $z=1/4+(1/2)w$.
Substituting back into the first row and solving for $x$ leaves
$x=-(7/8)+(1/2)y+(1/4)w$.
\end{frame}
