...
\ex
Consider the projection map $\map{\pi}{\Re^2}{\Re}$ defined by
\begin{equation*}
  \pi(\colvec{x \\ y})
  =x
\end{equation*}
that just ``forgets'' the second component.
The check that $\pi$ is a homomorphism is routine.
\end{frame}
\begin{frame}
This function is many-to-one; for example many vectors are mapped
to the member $2$ of the codomain.
\centergraphic{asy/three_ii_proj.pdf}
\end{frame}
