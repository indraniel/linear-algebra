...
\title[Definition of Vector Space] % (optional, use only with long paper titles)
{Two.I Definition of Vector Space}
...
\subject{Definition of Vector Space}
...
% \begin{enumerate}
% \item
% their \definend{vector sum}\index{vector!sum}\index{sum!vector}%
...
% \begin{enumerate}
% \setcounter{enumi}{1}
% \item \( \vec{v}+\vec{w}=\vec{w}+\vec{v} \) and 
...
We first check \definend{closure under addition}~(1),
...
and note that it is the \definend{identity element}
...
\end{align*}
\end{frame}\begin{frame}
Condition~(5), \definend{existence of an additive inverse}, is also a matter of 
...
First, condition~(6) is \definend{closure under scalar multiplication}.
...
\pause
Condition~(7) is that 
\definend{real number addition distributes over scalar multiplication}.
...
\end{frame}\begin{frame}
For 
\definend{distributivity of vector addition over scalar multiplication}~(8),
