...
We can draw two-unknkown equations as lines.
...
\begin{frame}{Vectors}
A \definend{vector} is an object that can be specified completely by
giving its magnitude and its direction.
...
Think of a vector as representing a displacement so each of the 
above is a displacement of one over and two up.
\end{frame}
...
\begin{equation*}
  \colvec{1 \\ 2}
\end{equation*}
We often draw the vector
\begin{equation*}
  \vec{v}=\colvec{v_1 \\ v_2}
\end{equation*}
as starting at the origin and extending to $(v_1,v_2)$ and we may refer to it
as ``the point $\vec{v}\,$''
...
Scalar multiplication affects the length of a vector but not its direction.
...
In higher dimensions lines work the same way.
\end{frame}
...
\pause
For any nonzero vector $\vec{v}$, the vector with the same direction
and length one is $\vec{v}/\norm{\vec{v}}$.
We say that this
...
We will motivate that definition with
two vectors in \( \Re^3 \). 
\centergraphic{../ch1.21}
Put them at the origin and in the 
...
% ..........
\begin{frame}
Note that two vectors are orthogonal if and only if their dot product is zero.  
\end{frame}
