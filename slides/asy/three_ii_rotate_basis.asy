...
draw(e1,VECTORPEN,Arrow(DefaultHead,.7*VECTORHEADSIZE));
draw(rotate(theta)*e1,VECTORPEN+red,Arrow(DefaultHead,.7*VECTORHEADSIZE));
draw(e2,VECTORPEN,Arrow(DefaultHead,.7*VECTORHEADSIZE));
draw(rotate(theta)*e2,VECTORPEN+red,Arrow(DefaultHead,.7*VECTORHEADSIZE));
// Label
label("\tiny $\colvec{\cos\Theta \\ \sin\Theta}$",rotate(theta)*a1,E);
label("\tiny $\colvec{-\sin\Theta \\ \cos\Theta}$",rotate(theta)*a2,NW);
