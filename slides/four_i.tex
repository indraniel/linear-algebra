...
\end{align*}
\end{frame}
\begin{frame}That example illustrates that determinants are not linear
with respect to scalar multiplication.
What about with respect to addition?

Consider condition~(3) in the case   
% \begin{equation*}
...
This strongly hints that besides bringing out scalars one row at a time,
...
We are left with two matrices, each having 
...
\end{equation*}
by breaking it into
a sum of determinants of matrices.
These summand matrices will be all~$0$'s except that each
row will have one entry from the 
starting matrix.   
\begin{equation*}
  =\begin{vmat}
    1 &0 &0 \\
...
since one row is a multiple of the other.
