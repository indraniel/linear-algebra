...
That is, the sum of two linear functions
$\map{f,g}{V}{W}$
\begin{equation*}
...
is linear,
and 
the scalar multiple $r\cdot f$ of a linear function (where $r\in\Re$)
\begin{equation*}
...
\pause
We first do an exploratory calculation.
\ex
...
\end{equation*}
So, on the same input vector, compared with the representation of the output
vector from~$f$, the 
...
\begin{frame}
\noindent We next do a similar exploratory calculation for addition.
