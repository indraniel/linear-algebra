...
\pause
We now see how the matrix representations
s $\rep{f}{B,D}$ and 
$\rep{g}{B,D}$
combine to give the representations $\rep{r\cdot f}{B,D}$
and $\rep{f+g}{B,D}$.
\end{frame}
\begin{frame}
\ex
Let $\map{f}{V}{W}$ be the linear map represented 
in this way with respect to 
some bases $B=\sequence{\vec{\beta}_1,\vec{\beta}_2}$
...
\begin{equation*}
  ...
  \end{mat}
\end{equation*}
and suppose that where $f(\vec{v})=\vec{w}$,
...
\end{equation*}
We will find the matrix representing $6f$.

The action of $6f$ is 
$\vec{v}\mapsto 6\vec{w}=6(w_1\vec{\delta}_1+w_2\vec{\delta}_2)
=(6w_1)\vec{\delta}_1+(6w_2)\vec{\delta}_2$,
so the output vector has
this representation.
\begin{equation*}
  % \rep{6f(\vec{v})}{D}
  % =
...
\end{equation*}
Thus,
compared with the representation of the output
vector from~$f$, the 
representation of the output 
vector from $6f$ has entries that are
$6$ times as big.

% Then how do the matrices representing each function compare? 
\end{frame}
\begin{frame}
\noindent Here is the matrix-vector representation of the action of~$f$.  
\begin{equation*}\
...
\end{equation*}
\pause
The prior slide shows that on the same $\vec{v}$ the matrix representing $6f$ 
does this.
\begin{equation*}
  \rep{6f}{B,D}\cdot
  \colvec{v_1 \\ v_2}
...
\end{equation*}
Therefore this is the matrix representing the map $\map{6f}{V}{W}$.
\begin{equation*}
  \rep{6f}{B,D}
  =
  \begin{mat}
  ...
  \end{mat}
\end{equation*}
Going from the function $f$ to the 
...
\begin{frame}
\ex
Suppose that $\map{f,g}{V}{W}$ are linear maps represented by these matrices.
\begin{equation*}
  \rep{f}{B,D}=
  \begin{mat}
    2  &1  \\
  ...
  \end{mat}
\end{equation*}
Let $f(\vec{v})=\vec{u}$ and $g(\vec{v})=\vec{w}$ 
with these the column vector representations.
\begin{equation*}
  \rep{f(\vec{v})}{D}=\colvec{u_1 \\ u_2}
  \quad
  \rep{g(\vec{v})}{D}=\colvec{w_1 \\ w_2}
\end{equation*}
We will find the matrix representing $f+g$.
...
\end{align*}
Thus for any $\vec{v}$, the effect on the representations
...
\end{frame}
\begin{frame}
\noindent The functions given have 
this effect.
\begin{align*}
  \rep{f(\vec{v})}{D}=
  \begin{mat}
...
\end{align*}
\pause
So we want the matrix that acts in this way.
\begin{equation*}
  \rep{\,(f+g)\,(\vec{v})\,}{D}
  \colvec{v_1 \\ v_2}
...
\begin{equation*}
  ...
  \end{mat}
\end{equation*}
So, in this exploratory example,
...
Consider the order in which we write a composition.
In $\composed{g}{h}$ the function $g$ is written first, or leftmost, 
but it is the one applied second.
\begin{equation*}
...
\setlength{\fboxsep}{1.5pt}
    \begin{mat}
       ...
       {\color{green} \begin{array}{@{}cc@{}} 0  &1 \end{array}}           \\ 
    ...
    \begin{mat}
      ...
      &{\color{green}\begin{array}{@{}c@{}}  6  \\  7  \end{array}}
