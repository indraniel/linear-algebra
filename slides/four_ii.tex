...
\begin{equation*}
  ...
  \end{mat}
\end{equation*}
will, by the theorem, change the size of a box by a factor of $\deter(T)=-2$.
Here is an example. 
\end{frame}
\begin{frame} 
The box defined by the two vectors $\vec{v}_1=\binom{1}{0}$ and
$\vec{v}_2=\binom{1}{1}$ is transformed by $t$ to the box defined by the 
two vectors $t(\vec{v}_1)=\binom{1}{3}$ and 
$t(\vec{v}_2)=\binom{3}{7}$. 
\begin{center}
  \vcenteredhbox{\includegraphics{asy/four_ii_2dtransedsize0.pdf}}
  \quad$\mapsunder{t}$\quad
