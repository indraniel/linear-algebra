...
\index{vanishing point}\index{projection!central!vanishing point}%
...
%So the artist is trying to depict the room not as it is, but instead 
%as it appears.
...
\end{center}
%Although the three are not exactly the same geometric actions, 
...
For a better model, put the projector $P$ at the origin.
...
are said to be \emph{antipodal}.\index{antipodal}
...
in $\Re^3$
\begin{equation*}
  %\set{y\colvec{1 \\ -1 \\ 0}+z\colvec{1 \\ 0 \\ 1}\suchthat y,z\in\Re}=
...
\begin{exercises}
  ...
  \item 
    ...
    %such that it would project to to a circle)? 
    \begin{answer}
