...
The code we will use is in the \inlinecode{img_squeeze} function 
...
zero.\footnote{These are four values of $1.02472345283\protect\times 10^{-11}$
and one of $2.32533986491\protect\times 10^{-12}$.}
% \begin{lstlisting}
...
Its advantage is that it has only $20\%$ of the storage
and transmission requirements of the original image.

Where the image is $\nbyn{n}$, every additional term adds to the
storage and transmission requirements by about $2n$ reals.
The original image requires $n^2$ reals we want to choose percentage values 
less than $0.50$.
...
\lstinputlisting[firstline=37,lastline=49]{plot_action.sage}
...
\lstinputlisting[firstline=8,lastline=23]{img_squeeze.sage}
