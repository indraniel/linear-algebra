...
\subsectionoptional{Laplace's Expansion Formula}
\index{Laplace expansion|(}

An example shows it best so we will
illustrate with a $\nbyn{3}$ case, but the approach works for
taking the determinant of any square matrix.
...
\begin{align*}
  ...
  &=\begin{aligned}[t]
    ...
    \end{aligned}
\end{align*}
Factor out the entries from the first row $t_{1,1}$, $t_{1,2}$, $t_{1,3}$.
%</LaplaceExample0>
%<*LaplaceExample1>
\begin{align*}
...
This takes one swap to each of the permutation matrices on
...
%</LaplaceExample2>
%<*LaplaceExample3>
The point of the swapping is that 
...
We finish by applying this result to
