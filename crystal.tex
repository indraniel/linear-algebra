...
\topic{Crystals}
\index{crystals|(}
Everyone has noticed that table salt\index{crystals!salt}
comes in little cubes.
\begin{center}
  \scannedpicture{salt} %1.25in tall
  %\includegraphics{salt.eps} %1.25in tall
\end{center}
Remarkably, the explanation for the cubical external shape is the simplest
one:~the internal shape, the way the atoms lie, is also cubical.
...
\end{center}
The specks of salt that we see when we spread a little out on the 
table consist of many repetitions of this fundamental unit. 
That is, these cubes of atoms stack up to make the
larger cubical structure that we see.
...
There,
we have the square repeated many times.
\begin{center}
...
Obviously that unit is unwieldly. 
Instead, the thing to do is to 
take as a unit the length of each side of the square.
...
It is graphite,\index{crystals!graphite} 
formed from carbon atoms arranged in this shape.
\begin{center}  %graphite
  \includegraphics{ch2.10}
\end{center}
This is a single plane of graphite.
A piece of graphite consists of many of these planes layered in a stack.
...
We can get a convienent unit of length by
...
\end{center}
Then a natural basis consists of the vectors that form the sides of
that unit cell.
...
The selection of convienent bases extends to three dimensions.
...
cube is more complicated than salt's. 
...
\end{center}
(To show the added face carbons clearly, 
the corner carbons have been reduced to dots.)
...
\begin{exercises}
  ...
  \item 
    ...
    \begin{exparts}
      ...
      \partsitem Platinum crystalizes in a face-centered cubic lattice
