...
With those two we computed the values of the unknown masses.
Had we instead gotten the second balance at
\( 20 \)~cm and \( 7.5 \)~cm
then we would not have been able to solve for the unknown values.
Instead Gauss's Method on the resulting two-equations, two-unknowns system
would have yielded a $0=0$ equation, along with an
equation containing a free variable.
Intuitively, the problem would have been that \( \rowvec{20 &7.5} \) is half of 
$\rowvec{40 &15}$, that is,
$\rowvec{20 &7.5}$ is in the span of the set $\set{\rowvec{40 &15}}$
and so is repeated information.
...
that it depends on, is expressible in terms of, 
elements of the set 
...
The discussion at the section's start involved removing vectors instead of
adding them. 
...
the definition requires a multiset, strictly speaking.}\spacefactor=1000\ 
...
first two, namely, we get $c_1\vec{v}_1+c_2\vec{v}_2=\zero$ 
...
% This section began with a discussion and example 
...
% It supports the
% definition of linearly independent set because it says that
% such a set is a minimal spanning set in that we cannot 
% omit any element without changing the span.
...
$\spanof{S-\set{v}}\subset\spanof{S}$.
\end{corollary}
...
The next one addresses adding elements.
...
Suppose first that $v\in\spanof{S}$ so that
$\vec{v}=c_1\vec{s}_1+\cdots+c_n\vec{s}_n$.
Then $\zero=c_1\vec{s}_1+\cdots+c_n\vec{s}_n-1\cdot\vec{v}$ is 
a nontrivial linear dependence among the elements of $S\union\set{\vec{v}}$,
so that set is not linearly independent.
