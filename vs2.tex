...
If $\vec{v}\not\in S$ then:
(1)~$\spanof{S}\subsetneq \spanof{S\union\set{\vec{v}}}$ if and only if 
  $\vec{v}\not\in \spanof{S}$
and 
(2)~if $S$ is linearly independent then adjoining $\vec{v}$ to $S$ gives a
  linearly independent set if and only if $\vec{v}\not\in \spanof{S}$.
\end{lemma}

\begin{proof}
We have $\spanof{S}\subseteq \spanof{S\union\set{\vec{v}}}$ because an
element of $\spanof{S}$ is a linear combination of elements of $S$,
so is a linear combination of elements of $S\union\set{\vec{v}}$,
and therefore is an element of $\spanof{S\union\set{\vec{v}}}$.

Now assume $\vec{v}\not\in S$.

For~(1), one direction of the if and only if is easy: if 
$\vec{v}\not\in \spanof{S}$
then because $\vec{v}\in\spanof{S\union\set{\vec{v}}}$ we have that
$\spanof{S}\neq\spanof{S\union\set{\vec{v}}}$. 
For the other direction, if $\vec{v}\in\spanof{S}$ then $\vec{v}$ is a linear
combination of elements of $S$ so
since any member of $\spanof{S\union\set{\vec{v}}}$ has the form
$d_1\vec{s}_1+\cdots+d_n\vec{s}_n+d_{n+1}\vec{v}$, expanding
$\vec{v}$ to its linear combination in terms of members of $S$ makes it a 
linear combination of members of $S$, and therefore a member of $\spanof{S}$.  

For~(2), assume first that $\vec{v}\in \spanof{S}$.
Then $\vec{v}=c_1\vec{s}_1+\cdots+c_n\vec{s}_n$ and adjoining $\vec{v}$
to $S$ gives this linearly dependendence among members of 
the multiset $S\union\set{\vec{v}}$:
$c_1\vec{s}_1+\cdots+c_n\vec{s}_n-\vec{v}=\zero$.
If $\vec{v}\not\in\spanof{S}$ then assuming that 
\( S\union\set{\vec{v}} \) is linearly dependent gives a
nontrivial \( c_0\vec{v}+\lincombo{c}{\vec{s}}=\zero \)
and the linear independence of $S$ implies that \( c_0\neq 0 \) 
or else this would be a nontrivial relationship among members of \( S \).
