...
As that example shows, to understand whether adding a vector to a set will
...
since any $\vec{w}\in\spanof{S\union\set{\vec{v}}}$ has the form
$\vec{w}=d_{0}\vec{v}+d_1\vec{s}_1+\cdots+d_n\vec{s}_n$, expanding
...
\begin{equation*}
  S
   =\set{\colvec{1 \\ 0 \\ 0}}
\end{equation*}
Note that the span of $S$ is the $x$-axis.
...
\begin{center}
     ...
     \( \set{
         ...
         \colvec{0 \\ 0 \\ 1} } \)    
\end{center}
Again, the additional member of the 
dependent superset comes from the $xy$-plane, while the 
additional member of the
independent superset comes from outside of that plane.

For this independent set \( \spanof{S}=\Re^3 \).
\begin{equation*}
   S =\set{\colvec{1 \\ 0 \\ 0},
          \colvec{0 \\ 1 \\ 0},
          \colvec{0 \\ 0 \\ 1}        } 
...
\begin{theorem} \label{th:AlwaysAnLDSubset}
%<*th:AlwaysAnLDSubset>
In a vector space,
...
(since adding it would not cause the span to grow).
%</pf:AlwaysAnLDSubset1>
...
The fourth case, whether linear independence is preserved by superset.
is covered by \nearbyexample{ex:LinindSetsAndSuper}, which gives instances
where a linearly independent set has both a linearly independent superset
and one that is linearly dependent.
...
\nearbyexample{ex:LinindSetsAndSuper} also shows that some 
