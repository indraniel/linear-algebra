...
If $\vec{v}\not\in\spanof{S}$, if adjoining $\vec{v}$ to $S$ 
resulted in a nontrivial linear relationship
\( c_0\vec{v}+\lincombo{c}{\vec{s}}=\zero \)
then because the linear independence of $S$ implies that \( c_0\neq 0 \) 
...
Therefore if $\vec{v}\not\in\spanof{S}$ then 
the only such linear relationship is trivial. 
Conversely, if $\vec{v}\in \spanof{S}$
then there is a dependence $\vec{v}=c_1\vec{s}_1+\cdots+c_n\vec{s}_n$ 
