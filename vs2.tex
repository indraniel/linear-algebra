...
three members while we've given spanning sets with two.
...
% %<*lm:VecInSpanIffSpanUnchByAddVec>
...
% \begin{proof}
% %<*pf:VecInSpanIffSpanUnchByAddVec0>
...
% %<*pf:VecInSpanIffSpanUnchByAddVec1>
...
Recall the Statics example from the opening 
of \hyperlink{ex:Statics}{Section~One.I}.
We first got a balance with the unknown-mass objects 
at \( 40 \)~cm and \( 15 \)~cm 
and then got another balance at \( -50 \)~cm and \( 25 \)~cm.
With those two pieces of information we could compute values of the unknown
masses.
However, had we instead gotten the second balance at
\( 20 \)~cm and \( 7.5 \)~cm
then we would not be able to find the unknown values.
The difficulty is that the \( \rowvec{20 &7.5} \) information is a ``repeat'' 
of the
$\rowvec{40 &15}$ information.
That is, the difficulty is that
$\rowvec{20 &7.5}$ is in the span of the set $\set{\rowvec{40 &15}}$ 
and so we would be trying to 
solve a two-unknowns problem with essentially one piece of information.
\end{example}
 
As that example shows, to know whether adding a vector to a set will
increase the span or conversely whether removing that vector will decrease
the span, we need to know whether the vector is a linear combination
of other members of the set. 
...
others.\appendrefs{multisets}\spacefactor=1000\ 
...
This is easy to check in this case, but also 
...
first two, namely, we can satisfy $c_1\vec{v}_1+c_2\vec{v}_2=\zero$ 
by taking $c_1=0$ and $c_2=0$
...
Here is an example showing that we can need multiset rather than set
(recall that in a set repeated elements collapse so that the set 
$\set{0,1,0}$ equals the set $\set{0,1}$, whereas in a multiset they
do not collapse so that the multiset $\set{0,1,0}$ contains the element 
$0$ twice).
...
Because in a set repeated elements collapse to be a single element 
these images form the one-element set $\set{1}$,
...
This section began with a discussion and an example 
about when a set contains ``repeat'' elements, ones that we can 
omit without shrinking the span.
The next result characterizes when this happens.
And, it supports the
definition of linear independence because it says that
such a set is a minimal spanning set in that we cannot 
omit any element without changing its span.

\begin{lemma} \label{lm:ShrinkSpanByRemovingNonRepeat}
%<*lm:ShrinkSpanByRemovingNonRepeat>
If $\vec{v}$ is a member of a vector space $V$ and $S\subseteq V$ 
then $\spanof{S-\set{\vec{v}}}\subseteq \spanof{S}$.
Also: (1)~if $\vec{v}\in S$ then
$\spanof{S-\set{\vec{v}}}=\spanof{S}$ if and only if 
  $\vec{v}\in\spanof{S-\set{\vec{v}}}$
and 
(2)~the condition that 
removal of any $\vec{v}\in S$ shrinks the span 
$\spanof{S-\set{\vec{v}}}\neq \spanof{S}$
holds if and only if $S$ is linearly independent. 
%</lm:ShrinkSpanByRemovingNonRepeat>
\end{lemma}

\begin{proof}
%<*pf:ShrinkSpanByRemovingNonRepeat0>
First, $\spanof{S-\set{\vec{v}}}\subseteq \spanof{S}$ 
because an element of $\spanof{S-\set{\vec{v}}}$ 
is a linear combination of elements of $S-\set{\vec{v}}$,
and so is a linear combination of elements of $S$,
and so is an element of $\spanof{S}$.
%</pf:ShrinkSpanByRemovingNonRepeat0>

%<*pf:ShrinkSpanByRemovingNonRepeat1>
For statement~(1), one half of the if and only if is easy: 
if $\vec{v}\not\in\spanof{S-\set{\vec{v}}}$ then
$\spanof{S-\set{\vec{v}}}\neq \spanof{S}$ since the set on the right
contains $\vec{v}$ while the set on the left does not.
%</pf:ShrinkSpanByRemovingNonRepeat1>

%<*pf:ShrinkSpanByRemovingNonRepeat2>
The other half of the if and only if assumes that 
$\vec{v}\in\spanof{S-\set{\vec{v}}}$,
so that it is a combination 
$\vec{v}=c_1\vec{s}_1+\cdots +c_n\vec{s}_n$ of members of 
$S-\set{\vec{v}}$.
To show that $\spanof{S-\set{\vec{v}}}= \spanof{S}$, by the first paragraph
we need only show that each element of $\spanof{S}$ 
is an element of
$\spanof{S-\set{\vec{v}}}$.
So consider a linear combination
$d_1\vec{s}_{n+1}+\cdots+d_m\vec{s}_{n+m}+d_{m+1}\vec{v}\in\spanof{S}$ 
(we can assume that each $\vec{s}_{n+j}$ is unequal to $\vec{v}$).
Substitute for $\vec{v}$
\begin{equation*}
  d_1\vec{s}_{n+1}+\cdots+d_m\vec{s}_{n+m}+d_{m+1}(c_1\vec{s}_1+\cdots +c_n\vec{s}_n)
\end{equation*}
to get a linear combination of linear combinations of members of 
$\spanof{S-\set{\vec{v}}}$, which 
is a member of $\spanof{S-\set{\vec{v}}}$.
%</pf:ShrinkSpanByRemovingNonRepeat2>

%<*pf:ShrinkSpanByRemovingNonRepeat3>
For statement~(2) assume first that $S$ is linearly
independent and that $\vec{v}\in S$.
If removal of $\vec{v}$ did not shrink the span, so that
$\vec{v}\in\spanof{S-\set{\vec{v}}}$, then 
we would have $\vec{v}=c_1\vec{s}_1+\cdots+c_n\vec{s}_n$, 
which would be a linear dependence
among members of $S$, contradicting that $S$ is independent.
Hence $\vec{v}\not\in\spanof{S-\set{\vec{v}}}$
and the two sets are not equal.
%</pf:ShrinkSpanByRemovingNonRepeat3>

%<*pf:ShrinkSpanByRemovingNonRepeat4>
Do the other half of this if and only if statement by assuming
that $S$ is not linearly independent, so that some linear dependence 
$\vec{s}=c_1\vec{s}_1+\cdots+c_n\vec{s}_n$ holds among its members
(with no $\vec{s}_i$ equal to $\vec{s}$).
Then $\vec{s}\in\spanof{S-\set{\vec{s}}}$ and by statement~(1) its
removal will not shrink the span
$\spanof{S-\set{\vec{s}}}= \spanof{S}$.
%</pf:ShrinkSpanByRemovingNonRepeat4>
\end{proof}
...
\begin{lemma}  \label{lm:ExpandSpanByAddingNonRepeat}
%<*lm:ExpandSpanByAddingNonRepeat>
If $\vec{v}$ is a member of the vector space $V$ and $S$ is a subset
of $V$
then $\spanof{S}\subseteq \spanof{S\union\set{\vec{v}}}$.
Also:
(1)~adding $\vec{v}$ to $S$ does not increase the span 
  $\spanof{S}=\spanof{S\union\set{\vec{v}}}$ if and only if 
  $\vec{v}\in \spanof{S}$,
and 
(2)~if $S$ is linearly independent then adjoining $\vec{v}$ to $S$ gives a
  set that is also
  linearly independent if and only if $\vec{v}\not\in \spanof{S}$.  
%</lm:ExpandSpanByAddingNonRepeat>
\end{lemma}

\begin{proof}
%<*pf:ExpandSpanByAddingNonRepeat0>
The first sentence and statement~(1) are translations of the first
sentence and statement~(1) from the prior result.
%</pf:ExpandSpanByAddingNonRepeat0>

%<*pf:ExpandSpanByAddingNonRepeat1>
For statement~(2) assume that $S$ is linearly independent.
Suppose first that $\vec{v}\not\in\spanof{S}$. 
If adjoining $\vec{v}$ to $S$ 
resulted in a nontrivial linear relationship
\( \lincombo{c}{\vec{s}}+c_{n+1}\vec{v}=\zero \)
then because the linear independence of $S$ implies that \( c_{n+1}\neq 0 \) 
(or else the equation would be a nontrivial 
relationship among members of \( S \)),
we could rewrite the relationship as
\( \vec{v}=-(c_1/c_{n+1})\vec{s}_1-\dots-(c_n/c_{n+1})\vec{s}_n \) to get
the contradiction 
...
%<*pf:ExpandSpanByAddingNonRepeat2>
Conversely, if we suppose that $\vec{v}\in \spanof{S}$
then there is a dependence 
$\vec{v}=c_1\vec{s}_1+\cdots+c_n\vec{s}_n$ ($\vec{s}_i\in S$)
inside of
$S$ with $\vec{v}$ adjoined.
%</pf:ExpandSpanByAddingNonRepeat2>
\end{proof}
...
% then $\spanof{S}\subseteq \spanof{S\union\set{\vec{v}}}$.
% Also:
% (1)~adding $\vec{v}$ to a set strictly increases the span 
%   $\spanof{S}\subsetneq \spanof{S\union\set{\vec{v}}}$ if and only if 
%   $\vec{v}\not\in \spanof{S}$,
% and 
% (2)~if $S$ is linearly independent then adjoining $\vec{v}$ to $S$ gives a
%   set that is also
...
% \begin{proof}
% %<*pf:ExpandSpanByAddingNonRepeat0>
% First, $\spanof{S}\subseteq \spanof{S\union\set{\vec{v}}}$ because an
% element of $\spanof{S}$ is a linear combination of elements of $S$,
% and so is a linear combination of elements of $S\union\set{\vec{v}}$,
% and so is an element of $\spanof{S\union\set{\vec{v}}}$.
% %</pf:ExpandSpanByAddingNonRepeat0>

% %<*pf:ExpandSpanByAddingNonRepeat1>
% For statement~(1), one half of the if and only if is easy: if 
% $\vec{v}\not\in \spanof{S}$
% then because $\vec{v}\in\spanof{S\union\set{\vec{v}}}$ we have that
% $\spanof{S}\neq\spanof{S\union\set{\vec{v}}}$. 
% Conversely, if $\vec{v}\in\spanof{S}$ then 
% for any $\vec{w}\in\spanof{S\union\set{\vec{v}}}$ we can write
% $\vec{w}=d_{0}\vec{v}+d_1\vec{s}_1+\cdots+d_n\vec{s}_n$, and expanding
% $\vec{v}$ as a linear combination of members of $S$ 
% gives $\vec{w}$ as a 
% linear combination of members of $S$, and therefore as 
% a member of $\spanof{S}$.  
% %</pf:ExpandSpanByAddingNonRepeat1>
...
% Therefore if $\vec{v}\not\in\spanof{S}$ then 
% the only linear relationship is trivial. 
% Conversely, if we assume that $\vec{v}\in \spanof{S}$
% then $\vec{v}=c_1\vec{s}_1+\cdots+c_n\vec{s}_n$ and so there is a dependence  
...
\begin{center}
     ...
     \( \set{
         ...
         \colvec{0 \\ 1 \\ 0}} \)      
\end{center}
This illustrates \nearbylemma{lm:ExpandSpanByAddingNonRepeat}: we got the
dependent superset by adding a vector in the $x$-axis
and so the span did not grow, 
while we got the independent superset by adding a vector 
that isn't in $\spanof{S}$ because it has 
a nonzero $y$~component.
...
dependent superset comes from $\spanof{S}$, here the $xy$-plane, while the 
additional member of the
independent superset comes from outside of that plane.

Now consider this independent set \( \spanof{S}=\Re^3 \).
\begin{equation*}
   S =\set{\colvec{1 \\ 0 \\ 0},
          \colvec{0 \\ 1 \\ 0},
          \colvec{0 \\ 0 \\ 1}        } 
...
\begin{center}
     ...
     \( \set{
         ...
         \colvec{2 \\ -1 \\ 3} } \)     
\end{center}
but there is no linearly independent superset.
...
Another way to see it is \nearbylemma{lm:ExpandSpanByAddingNonRepeat}\Dash
we cannot add any vectors from outside of the span $\spanof{S}$ because that
span is all of $\Re^3$.
\end{example}

% \nearbylemma{lm:ExpandSpanByAddingNonRepeat} says that
% if we have a set that is linearly dependent then we can, without changing
% the span,  
% discard what \nearbyexample{ex:StaticsLIAndLD}
% calls ``repeat'' vectors.
...
By the definition of dependence, $S$ contains
a vector $\vec{v}_1$ that is a linear combination of
the others.
Define the set \( S_1=S-\set{\vec{v}_1} \).
By \nearbylemma{lm:ShrinkSpanByRemovingNonRepeat} the span does not
shrink: \( \spanof{S_1}=\spanof{S} \)
(since adding $\vec{v}_1$ to $S$ would not cause the span to grow).
%</pf:AlwaysAnLDSubset1>

%<*pf:AlwaysAnLDSubset2>
Now, if \( S_1 \) is linearly independent then we are done.
Otherwise iterate: 
...
\begin{equation*}
  S=\set{\colvec[r]{1 \\ 0 \\ 0},
     ...
     \colvec[r]{3 \\ 3 \\ 0}  }
\end{equation*}
We will find vectors to drop to get a set that is independent.
...
\end{equation*}
If we set one of the free variables to $1$, and the other to $0$,
then we get 
$c_1=-3$, $c_2=-3/2$, and $c_4=0$. 
We have this instance of ($*$).
\begin{equation*}
  ...
  =\colvec[r]{0 \\ 0 \\ 0}
\end{equation*}
Thus the vector associated with the free variable~$c_5$
is in the span of the set of vectors associated with the leading
variables $c_1$ and~$c_2$.
\nearbylemma{lm:ShrinkSpanByRemovingNonRepeat} says that we can
discard the fifth vector without shrinking the span.

Similarly, in the parametrization of the solution set let
$c_3=1$, and $c_5=0$,
to get an instance of~($*$)
showing that we can discard the third vector without shrinking the span. 
...
\begin{equation*}
  S=\set{\colvec[r]{1 \\ 0 \\ 0},
     ...
     \colvec[r]{0 \\ -1 \\ 1}  }
\end{equation*}
has the same span as $S$.
We can easily check that it is linearly independent  
and so discarding any of its elements will shrink the span.
\end{example}

% The general method is: 
% given a finite set of vectors, first
% write the system to express a linear relationship,
% apply Gauss's Method, and then
% discard the vectors 
...
% We can restate \nearbylemma{lm:ExpandSpanByAddingNonRepeat}
...
no vector exists that is not already in the span.
This nicely
complements the fact that \nearbylemma{lm:ShrinkSpanByRemovingNonRepeat}
shows that
...
The most important is \nearbylemma{lm:ExpandSpanByAddingNonRepeat}, which 
    ...
    \begin{answer}
      \begin{exparts}
        \partsitem A singleton set $\set{\vec{v}}$ is linearly independent 
          ...
          \nearbylemma{lm:ExpandSpanByAddingNonRepeat} where \( S=\emptyset \).)
